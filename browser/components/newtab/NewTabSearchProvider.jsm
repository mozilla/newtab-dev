...
SearchProvider.prototype = {
  ...
  removeFormHistory(target, suggestion) {
    ContentSearch.removeFormHistoryEntry({target: target.browser}, suggestion);
  },

  performSearch(target, searchData) {
    return Task.spawn(function* () {
      ContentSearch.performSearch({target: target.browser}, searchData);
      yield ContentSearch.addFormHistoryEntry({target: target.browser}, searchData.searchString);
    }.bind(this));
  },
  ...
  getSuggestions(engineName, searchString, target) {
    return Task.spawn(function* () {
