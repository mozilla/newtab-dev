...
add_task(function test_updateSuggestedTile() {
  ...
  let origGetFrecentSitesName = RemoteDirectoryLinksProvider.getFrecentSitesName;
  RemoteDirectoryLinksProvider.getFrecentSitesName = () => "";
  ...
  RemoteDirectoryLinksProvider.getFrecentSitesName = origGetFrecentSitesName;
...
add_task(function test_suggestedLinksMap() {
  let origGetFrecentSitesName = RemoteDirectoryLinksProvider.getFrecentSitesName;
  RemoteDirectoryLinksProvider.getFrecentSitesName = () => "testing map";
...
add_task(function test_suggestedAttributes() {
  let origIsTopPlacesSite = RemoteNewTabUtils.isTopPlacesSite;
  RemoteNewTabUtils.isTopPlacesSite = () => true;
  ...
  let gLinks = RemoteNewTabUtils.links;
  gLinks.addProvider(RemoteDirectoryLinksProvider);
  gLinks.populateCache();
  yield new Promise(resolve => {
    RemoteNewTabUtils.allPages.register({
      observe: _ => _,
      update() {
        RemoteNewTabUtils.allPages.unregister(this);
        resolve();
      }
    });
  });
  ...
  RemoteNewTabUtils.isTopPlacesSite = origIsTopPlacesSite;
  RemoteDirectoryLinksProvider._getCurrentTopSiteCount = origCurrentTopSiteCount;
  gLinks.removeProvider(RemoteDirectoryLinksProvider);
  RemoteDirectoryLinksProvider.removeObserver(gLinks);
});
...
add_task(function test_frequencyCappedSites_views() {
  ...
  function synthesizeAction(action) {
    RemoteDirectoryLinksProvider.reportSitesAction([{
      link: {
...
add_task(function test_reportSitesAction() {
  ...
  function sendPingAndTest(path, action, index) {
    ...
    RemoteDirectoryLinksProvider.reportSitesAction(sites, action, index);
    return deferred.promise;
  }
...
add_task(function test_RemoteDirectoryLinksProvider_fetchDirectoryOnShow() {
  yield promiseSetupRemoteDirectoryLinksProvider();

  // set lastdownload to 0 to make RemoteDirectoryLinksProvider want to download
  RemoteDirectoryLinksProvider._lastDownloadMS = 0;
  do_check_true(RemoteDirectoryLinksProvider._needsDownload);

  // download should happen on view
  yield RemoteDirectoryLinksProvider.reportSitesAction([], "view");
  do_check_true(RemoteDirectoryLinksProvider._lastDownloadMS != 0);
...
/*
add_task(function test_RemoteDirectoryLinksProvider_enhancedURIs() {
  let origGetFrecentSitesName = RemoteDirectoryLinksProvider.getFrecentSitesName;
  RemoteDirectoryLinksProvider.getFrecentSitesName = () => "";
  let origIsTopPlacesSite = RemoteNewTabUtils.isTopPlacesSite;
  RemoteNewTabUtils.isTopPlacesSite = () => true;
...
/* XXX: Failing
add_task(function test_RemoteDirectoryLinksProvider_getFrequencyCapReportSiteAction() {
  yield promiseSetupRemoteDirectoryLinksProvider();
  yield RemoteDirectoryLinksProvider.init();
  ...
  do_check_true(RemoteDirectoryLinksProvider._testFrequencyCapLimits("bar.com"));
  // report site action
  yield RemoteDirectoryLinksProvider.reportSitesAction([{
    link: {
...
add_task(function test_inadjecentSites() {
  ...
  let origCurrentTopSiteCount = RemoteDirectoryLinksProvider._getCurrentTopSiteCount;
  RemoteDirectoryLinksProvider._getCurrentTopSiteCount = () => {
    origCurrentTopSiteCount.apply(RemoteDirectoryLinksProvider);
    return 8;
  };

  // store oroginal inadjacent sites url
  let origInadjacentSitesUrl = RemoteDirectoryLinksProvider._inadjacentSitesUrl;

  // loading inadjacent sites list function
  function setInadjacentSites(sites) {
    let badSiteB64 = [];
    sites.forEach(site => {
      badSiteB64.push(RemoteDirectoryLinksProvider._generateHash(site));
    });
    let theList = {"domains": badSiteB64};
    let dataURI = 'data:application/json,' + JSON.stringify(theList);
    RemoteDirectoryLinksProvider._inadjacentSitesUrl = dataURI;
    return RemoteDirectoryLinksProvider._loadInadjacentSites();
  };
  ...
  do_check_true(RemoteDirectoryLinksProvider._avoidInadjacentSites);
  // make sure example.com is included in inadjacent sites list
  do_check_true(RemoteDirectoryLinksProvider._isInadjacentLink({baseDomain: "example.com"}));
  ...
  link = RemoteDirectoryLinksProvider._updateSuggestedTile();
  do_check_false(link);
  do_check_true(RemoteDirectoryLinksProvider._newTabHasInadjacentSite);

  // _handleLinkChanged must return true on inadjacent site
  do_check_true(RemoteDirectoryLinksProvider._handleLinkChanged({
    url: "http://someothersite.com",
    type: "history",
  }));
  // _handleLinkChanged must return false on ok site
  do_check_false(RemoteDirectoryLinksProvider._handleLinkChanged({
    url: "http://foobar.com",
    type: "history",
  }));
  ...
  do_check_false(RemoteDirectoryLinksProvider._avoidInadjacentSites);
  link = RemoteDirectoryLinksProvider._updateSuggestedTile();
  do_check_true(link);
  do_check_eq(link.url, "http://turbotax.com");
  do_check_false(RemoteDirectoryLinksProvider._newTabHasInadjacentSite);
  ...
  do_check_true(RemoteDirectoryLinksProvider._isInadjacentLink({baseDomain: "someothersite.com"}));
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({baseDomain: "bar.com"}));
  do_check_true(RemoteDirectoryLinksProvider._isInadjacentLink({url: "http://www.someothersite.com"}));
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({url: "http://www.bar.com"}));
  // try to crash _isInadjacentLink
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({baseDomain: ""}));
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({url: ""}));
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({url: "http://localhost:8081/"}));
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({url: "abracodabra"}));
  do_check_false(RemoteDirectoryLinksProvider._isInadjacentLink({}));

  // test _checkForInadjacentSites
  do_check_true(RemoteDirectoryLinksProvider._checkForInadjacentSites());

  // Cleanup
  gLinks.removeProvider(RemoteDirectoryLinksProvider);
  RemoteDirectoryLinksProvider._inadjacentSitesUrl = origInadjacentSitesUrl;
  RemoteDirectoryLinksProvider.getFrecentSitesName = origGetFrecentSitesName;
  RemoteNewTabUtils.isTopPlacesSite = origIsTopPlacesSite;
  RemoteNewTabUtils.getProviderLinks = origGetProviderLinks;
  RemoteDirectoryLinksProvider._getCurrentTopSiteCount = origCurrentTopSiteCount;
...
add_task(function test_reportPastImpressions() {
  ...
  // make RemoteDirectoryLinksProvider load json
  let loadPromise = Promise.defer();
  RemoteDirectoryLinksProvider.getLinks(_ => {loadPromise.resolve();});
  ...
  expectedImpressions = RemoteDirectoryLinksProvider._frequencyCaps[testUrl].totalViews;
