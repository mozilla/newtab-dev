...
MozContentSearch.prototype = {
  ...
  get onvisibleenginechange() {
    return this.__DOM_IMPL__.getEventHandler("onvisibleenginechange");
  },

  set onvisibleenginechange(handler) {
    this.__DOM_IMPL__.setEventHandler("onvisibleenginechange", handler);
  },
  ...
  _fireStateChangeEvent(engines) {
    Task.spawn(function* () {
      ...
      const event = new this._win.MozSearchEnginesChangeEvent("visibleenginechange", eventInit);
      this.__DOM_IMPL__.dispatchEvent(event);
    }.bind(this));
  },
